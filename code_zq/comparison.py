...
def data_generation_outlier3(n, a, b, c, d, e, f, g, h, i, j, seed=0, alpha=0.1, outlier='Central'):
    ...
    if outlier == 'Central':
        train[0][indices] = rng.uniform(2, 4, size=int(n1 * alpha))
        train[2][indices] = rng.uniform(1, 2, size=int(n1 * alpha))
    elif outlier == 'Radius':
        train[1][indices] = rng.uniform(1, 2, size=int(n1 * alpha))
        train[3][indices] = rng.uniform(1, 2, size=int(n1 * alpha))
    else:
        train[0][indices] = rng.uniform(2, 4, size=int(n1 * alpha))
        train[2][indices] = rng.uniform(1, 2, size=int(n1 * alpha))
        train[1][indices] = rng.uniform(1, 2, size=int(n1 * alpha))
...
def data_generation_outlier4(n, a, b, c, d, e, f, g, h, i, j, seed=0, alpha=0.1, outlier='Central'):
    ...
    if outlier == 'Central':
        train[0][indices] = train[0][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
        train[2][indices] = train[2][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
    elif outlier == 'Radius':
        train[1][indices] = train[1][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
        train[3][indices] = train[3][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
    else:
        train[0][indices] = train[0][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
        train[2][indices] = train[2][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
        train[1][indices] = train[1][indices] + 0.1 * np.random.standard_t(df=1, size=int(n1 * alpha))
