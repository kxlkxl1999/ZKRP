...
#'
#' \code{CRM()} is used to fit a linear regression model based on the Center Method(Billard, L. and Diday, E.2000)
...
#'
#' @details
#' The basic idea is to estimate models independently for each of center points of the interval variables. The coefficients of the model are estimated by least square method.
...
#' @examples
#' data(example3)
#' m1 <- CM(cbind(Sepal.Length_L, Sepal.Length_U) ~ Sepal.Width_L + Sepal.Width_U + Petal.Length_L + Petal.Length_U + Petal.Length_L + Petal.Length_U, data = example3)
#' m1
#' m1$coefficients
#' @seealso \code{\link{RMSE}} \code{\link{symbolic.r}}
#' @import stats
#' @export
CM <- function(formula = formula, data = data) {
  ...
  fitted_L <- (x.C %*% coef.C) - ((x.R %*% coef.C) / 2)
  fitted_U <- (x.C %*% coef.C) + ((x.R %*% coef.C) / 2)
